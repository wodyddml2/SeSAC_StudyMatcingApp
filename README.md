...
1. 디자인, 기획, BE 파트와 협업 프로젝트를 진행하며, 디자인 도구 Figma를 사용하고 각 파트와 커뮤니케이션을 경험하게 되었습니다. 
또한, 이번 프로젝트를 진행하며 커뮤니케이션의 중요성과 서버 측 값 변경을 대비한 유지보수성이 높은 코드를 고려할 수 있게 되었습니다.

2. 저번 앱스토어 출시 프로젝트(아맛다)에서 개선 및 적용 해보고싶은 부분이 있었습니다.
중복 코드 최소화, 모듈화, MVVM 패턴, 반응형 프로그래밍을 적용하며 가독성과 유지보수성이 높은 코드로 개선할 수 있게 되었습니다.

#### **Problem • Try**

1. Codable 프로토콜을 채택 받는 모델을 생성했는데 서버의 응답 값의 변경을 대비하는 예외처리를 해주지 않았습니다.
해결책으로, 디코딩을 커스텀 할 때 decodeIfPresent를 사용하여 응답 값에 대한 예외처리를 할 수 있도록 개선할 것입니다.

2. 함수를 호출할 때마다 비용이 높은 DateFormatter 인스턴스를 생성해 메모리를 낭비하는 문제를 겪게 되었습니다. 
Singleton 패턴을 적용해 비용을 최소화하는 개선이 필요하다고 생각합니다.
다만, 결합도를 높일 수 있기 때문에 필요한 부분에서만 Singleton 패턴을 적용하도록 주의

3. 이번 프로젝트를 진행하면서 기능들에 대한 예외처리를 통해 사용자의 불편함을 최소화 시켰습니다. 
하지만 인증 타이머가 흘러가는 중에 앱이 백그라운드 상태로 갔을 경우, 타이머에 대한 예외 처리가 부족한 부분이 아쉬움으로 남았습니다.
앞으로 백그라운드 상태에 대한 부분을 좀 더 고려하여 사용자들의 불편을 최소화하도록 노력해야겠다고 생각합니다. 
